...
def _load_models() -> None:
    models_dir = os.path.join("save_models")
...
#     )
#     return _material_to_response(db.get_material(mid))
...
#     from agents.agent_predictor.predictor import build_trajectory as _build
#     return _build(request)


# @app.get("/time/{material_id}", response_model=TimeEstimation)
# def estimate_time(material_id: int):
#     from agents.agent_predictor.predictor import estimate_time as _estimate
#     m = db.get_material(material_id)
#     if m is None:
#         raise HTTPException(status_code=404, detail="Material not found")
